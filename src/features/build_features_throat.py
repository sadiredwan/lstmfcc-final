...
"""
Builds MFCC feature dataset from throat wav signals dataset
Args:
...
if __name__ == '__main__':
	os.chdir('../../')
	train_data_path = os.getcwd() + '/data/raw/throat/train/'
	test_data_path = os.getcwd() + '/data/raw/throat/test/'
	...
	for label, fname in tqdm(zip(labels, filenames)):
		...
		if len(signal) > config.samplerate:
			samples = resample(signal, config.samplerate, n=20//6)
		else:
			samples = [signal]
	...
	if STRAT == 'raw':
		...
		pickle.dump(X_raw, open('data/processed/throat/train/X_raw.pickle', 'wb'), protocol=4)
	elif STRAT == 'wpd':
		X_wpd = np.array(X_wpd)
		X_wpd = np.reshape(X_wpd, (X_wpd.shape[0], X_wpd.shape[2], -1))
		pickle.dump(X_wpd, open('data/processed/throat/train/X_wpd_level'+sys.argv[2]+'.pickle', 'wb'), protocol=4)
	elif STRAT == 'cwt':
		X_cwt = np.array(X_cwt)
		pickle.dump(X_cwt, open('data/processed/throat/train/X_cwt_level'+sys.argv[2]+'.pickle', 'wb'), protocol=4)

	y_train = label_transform(y_train)
	label_index = y_train.columns.values
	y_train = y_train.values
	y_train = np.array(y_train)
	pickle.dump(y_train, open('data/processed/throat/train/y_train.pickle', 'wb'), protocol=4)
