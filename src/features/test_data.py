...
"""
Builds MFCC feature test dataset from acoustic wav signals test dataset
Args:
...
if __name__ == '__main__':
	...
	labels, filenames = grab_files(test_data_path)
	...
	for label, fname in tqdm(zip(labels, filenames)):
		signal, rate = librosa.load(os.path.join(test_data_path, label, fname))
	...
	if STRAT == 'raw':
		...
		pickle.dump(X_raw, open('data/processed/acoustic/test/X_raw.pickle', 'wb'), protocol=4)
	elif STRAT == 'wpd':
		X_wpd = np.array(X_wpd)
		X_wpd = np.reshape(X_wpd, (X_wpd.shape[0], X_wpd.shape[2], -1))
		pickle.dump(X_wpd, open('data/processed/acoustic/test/X_wpd_level'+sys.argv[2]+'.pickle', 'wb'), protocol=4)
	elif STRAT == 'cwt':
		X_cwt = np.array(X_cwt)
		pickle.dump(X_cwt, open('data/processed/acoustic/test/X_cwt_level'+sys.argv[2]+'.pickle', 'wb'), protocol=4)

	y_train = label_transform(y_train)
	label_index = y_train.columns.values
	y_train = y_train.values
	y_train = np.array(y_train)
	pickle.dump(y_train, open('data/processed/acoustic/test/y_test.pickle', 'wb'), protocol=4)
